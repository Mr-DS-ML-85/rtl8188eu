# RTL8188EU Linux Driver — Auto USB ID Patcher & Installer

> **Kernel 6.15+ / 7.x compatible** — Automatically detects your USB adapter ID, patches the driver source, compiles, and installs.
...
The stock `rtl8188eu` driver often fails to recognize newer USB dongle variants because their USB Product IDs are missing from the driver's device table. This script:

1. Scans all connected USB devices for Realtek (`0x0bda`) adapters
2. Automatically adds any missing IDs into `os_dep/usb_intf.c`
3. Applies kernel 6.15+ API compatibility patches (`del_timer_sync` → `timer_delete_sync`, `from_timer` → `timer_container_of`)
4. Compiles the module
5. Installs the driver and firmware
6. Blacklists conflicting drivers (`rtl8xxxu`)
7. Loads the module
...
- `lsusb` (from `usbutils`): `sudo apt install usbutils`
...
```bash
git clone https://github.com/Mr-DS-ML-85/rtl8188eu.git
cd rtl8188eu
chmod +x install.sh
sudo ./install.sh
```
...
# 2. Detect your USB ID
lsusb | grep -i 0bda

# 3. Apply kernel API patches manually
grep -rl "del_timer_sync" . | xargs sed -i 's/del_timer_sync/timer_delete_sync/g'
grep -rl "from_timer"     . | xargs sed -i 's/from_timer/timer_container_of/g'
...
sudo rm -f /lib/modules/$(uname -r)/kernel/drivers/staging/r8188eu/8188eu.ko
sudo rm -f /lib/firmware/rtlwifi/rtl8188eufw.bin
sudo rm -f /etc/modprobe.d/rtl8188eu-blacklist.conf
sudo depmod -a
```
...
| Problem | Fix |
|---|---|
| No `wlan0` after install | Run `sudo dmesg \| grep 8188` to see errors |
| `Operation not permitted` on dmesg | Use `sudo dmesg` |
| Compile error on kernel 7.x | Script auto-patches; re-run `sudo ./install.sh` |
| Adapter not detected by `lsusb` | Try a different USB port; check with `sudo lsusb` |
| `module verification failed` | Normal on custom kernels — module still works |
| Conflicts with `rtl8xxxu` | Script blacklists it automatically |
...
| USB ID | Device |
|---|---|
| `0bda:8179` | RTL8188EUS (standard) |
| `0bda:f179` | RTL8188EUS (variant) |
| `0bda:f149` | RTL8188EU 802.11n dongle |

> More IDs are added automatically by the installer at runtime.

---

## 🔑 Kernel API Compatibility

| Kernel | Old API (removed) | New API (used by this driver) |
|---|---|---|
| 6.15+ | `del_timer_sync()` | `timer_delete_sync()` |
| 6.15+ | `del_timer()` | `timer_delete()` |
| 6.15+ | `from_timer()` | `timer_container_of()` |
...
<div align="center">
Made with ❤️ by <a href="https://github.com/Mr-DS-ML-85">Mr-DS-ML-85</a>
</div>
